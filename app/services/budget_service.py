"""Budget service — set, query, and check category budgets."""
...
async def set_budget(
    db: AsyncSession, user_id: int, category: str, monthly_limit: float
) -> Budget:
    ...
    logger.info(
        "Budget set: user %d, %s = %.2f", user_id, category, monthly_limit
    )
    return budget


async def get_budgets(db: AsyncSession, user_id: int) -> list[Budget]:
    """Return all budgets for a user, sorted by category."""
    stmt = (
        select(Budget)
